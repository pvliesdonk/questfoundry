...
DEFAULT_TOOLS_OUTPUT = Path(__file__).parent.parent / "src/questfoundry/tools/generated.py"
...
def schema_to_python_type(
    ...
    # Handle nullable types: ["integer", "null"] or ["string", "null"]
    is_explicitly_nullable = False
    if isinstance(prop_type, list):
        non_null_types = [t for t in prop_type if t != "null"]
        if "null" in prop_type and len(non_null_types) == 1:
            prop_type = non_null_types[0]
            is_explicitly_nullable = True
        else:
            raise ValueError(
                f"Unsupported type union '{prop_type}' for field '{prop_name}'. "
                "Only ['type', 'null'] unions are supported."
            )
    ...
    if prop_type == "integer":
        ...
        # Field is nullable if explicitly marked or if optional without default
        is_nullable = is_explicitly_nullable or (not is_required and not has_default)

        if has_default:
            # Use schema-specified default
            default_val = format_default_value(prop["default"])
            field_args.insert(0, f"default={default_val}")
            type_anno = "int | None" if is_explicitly_nullable else "int"
        elif is_nullable:
            type_anno = "int | None"
    ...
    if prop_type == "string":
        ...
        if has_default:
            default_val = format_default_value(prop["default"])
            field_args.insert(0, f"default={default_val}")
            type_anno = "str | None" if is_explicitly_nullable else "str"
        elif is_nullable:
            type_anno = "str | None"
            field_args.insert(0, "default=None")
        else:
...
def schema_to_tool_params(schema: dict[str, Any]) -> dict[str, Any]:
    """Convert a JSON Schema to LLM tool parameters format.
    ...
    Returns:
        Tool parameters dict suitable for LLM tool definitions.
    """
...
def generate_tool_schema_code(schema_path: Path) -> tuple[str, str]:
    """Generate Python code for a tool parameter schema.

    Args:
        schema_path: Path to the JSON schema file.

    Returns:
        Tuple of (variable_name, python_code).
    """
    with schema_path.open() as f:
        schema = json.load(f)
    ...
    # Extract artifact type from filename (e.g., "dream" from "dream.schema.json")
    artifact_type = schema_path.stem.replace(".schema", "")
    var_name = f"SUBMIT_{artifact_type.upper()}_PARAMS"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate Pydantic models and tool schemas from JSON Schema files.",
    ...
    parser.add_argument(
        ...
        help=f"Pydantic models output file (default: {DEFAULT_OUTPUT})",
    )
    parser.add_argument(
        "--tools-output-file",
        type=Path,
        default=DEFAULT_TOOLS_OUTPUT,
        help=f"Tool schemas output file (default: {DEFAULT_TOOLS_OUTPUT})",
    )
    parser.add_argument(
        "--no-tools",
        action="store_true",
        default=False,
        help="Skip generating tool schemas (useful for tests)",
    )
    return parser.parse_args()


def main() -> int:
    """Generate models and tool schemas from all schema files.
    ...
    tools_output_file: Path = args.tools_output_file
    generate_tools: bool = not args.no_tools
    ...
    # Write Pydantic models file
    generated_code = "\n".join(all_code)
    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file.write_text(generated_code)
    print(f"Generated {output_file}")

    # Write tool schemas file (only if not skipped)
    if generate_tools:
        tools_generated_code = "\n".join(tools_code)
        tools_output_file.parent.mkdir(parents=True, exist_ok=True)
        tools_output_file.write_text(tools_generated_code)
        print(f"Generated {tools_output_file}")

    # Format with ruff
    if not format_with_ruff(output_file):
        print("Warning: ruff formatting failed for models", file=sys.stderr)
        return 1

    if generate_tools and not format_with_ruff(tools_output_file):
        print("Warning: ruff formatting failed for tools", file=sys.stderr)
        return 1
