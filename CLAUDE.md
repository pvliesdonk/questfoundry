...
- **Bot reviewer "follow-up" language is NOT permission to defer.** Bot reviewers (claude-code-review, gemini-code-assist, codex) routinely soften findings with phrases like *"low-priority follow-up"*, *"not a blocker"*, *"minor"*, *"worth a follow-up issue"*, *"nice to have"*, or *"can be addressed later"*. That framing assumes a human-driven review loop where someone will revisit the area soon. **In an agent-driven workflow there is no "later".** The next PR that touches this file may be weeks away; the agent that opens it has zero memory of the prior review. Treat every such finding as if it said *"do this now in this PR"*. Default action: apply the fix in the current PR while the file is open, the validator state is in your head, and the reviewer's framing is fresh. The global `~/.claude/CLAUDE.md` rule on this is in force; this bullet exists because the bots' soft language has tricked agents into deferring repeatedly. The only valid skips are: (a) genuinely out-of-scope (touches code outside the PR's stated goal — file an issue in the same session and link it), or (b) the reviewer is wrong (post a reply explaining why; do not silently skip).
