...
2. **Library Development** (`lib/python/`):
   ...
   - The library loads schemas and prompts from `spec/` at runtime
...
- **Never duplicate resources** between `spec/` and `lib/`
- The Python library in `lib/python/` loads schemas from `spec/03-schemas/` and prompts from
  `spec/05-prompts/` at runtime
- There is one single source of truth for each resource
