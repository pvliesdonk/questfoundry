...
def generate_artifacts_code(
    ...
    for artifact_id, artifact_ir in sorted(artifacts.items()):
        ...
        # Class docstring with lifecycle info
        doc_lines = [f'    """{artifact_ir.name} artifact.']
        if artifact_ir.lifecycle:
            doc_lines.append("")
            doc_lines.append(f"    Lifecycle: {' → '.join(artifact_ir.lifecycle)}")
            doc_lines.append(f"    Store: {artifact_ir.store}")
        doc_lines.append('    """')
        ...
        if artifact_ir.lifecycle:
            ...
            lines.append(f"    _lifecycle: list[str] = [{lifecycle_str}]")
            lines.append("")
        ...
        for field in required_fields:
            ...
            if field.description:
                lines.append(
                    f'    {field_name}: {py_type} = Field(..., description="{field.description}")'
                )
            else:
        ...
        for field in optional_fields:
            ...
            if field.description:
                lines.append(
                    f'    {field_name}: {py_type} | None = Field(default=None, description="{field.description}")'
                )
            else:
