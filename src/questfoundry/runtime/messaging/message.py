...
def _default_priority_for_type(message_type: MessageType) -> int:
    ...
    priority_map = {
        ...
        MessageType.NUDGE: MessagePriority.LOW,  # Nudges are advisory
    }
...
def create_nudge(
    ...
    *,
    ...
    Nudges are gentle reminders generated by the runtime when agent behavior
    deviates from playbook expectations. They are advisory, not mandatory.

    Args:
        from_agent: Agent or system sending the nudge (usually "runtime")
        to_agent: Agent receiving the nudge
        ...
        playbook_instance_id: Specific playbook execution
        phase_id: Current phase in playbook
        expected_output: What output was expected (for missing_output)
        current_state: Current observed state (for unexpected_state)
        turn_created: Current turn number
        ttl_turns: TTL (default 3 turns - nudges expire quickly)
    ...
    return create_message(
        ...
        turn_created=turn_created,
        ttl_turns=ttl_turns,
        priority=MessagePriority.LOW,  # Nudges are low priority, advisory
    )
