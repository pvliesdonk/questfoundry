...
class TestEdgeEvaluator:
    """Test EdgeEvaluator functionality."""

    @pytest.fixture
    def evaluator(self):
        return EdgeEvaluator()

    def test_evaluate_python_expression_true(self, evaluator):
        """Test evaluating true python expression."""
        state = {
            "tu_id": "TU-2025-042",
            "tu_lifecycle": "hot-proposed",
            "current_node": "test",
    ...
    def test_evaluate_bar_threshold(self, evaluator):
        """Test evaluating bar threshold."""
        state = {
            "tu_id": "TU-2025-042",
...
class TestGraphFactory:
    """Test GraphFactory functionality."""

    @pytest.fixture
    def factory(self):
        return GraphFactory()

    def test_load_loop(self, factory):
        """Test loading a loop."""
        loop = factory.load_loop("story_spark")

        assert loop is not None
        assert loop.id == "story_spark"

    def test_validate_topology(self, factory):
        """Test topology validation."""
        loop = factory.load_loop("story_spark")

        # Should not raise
        factory.validate_topology(loop)
    ...
    def test_create_all_loops(self, factory):
        ...
        for loop_id in loop_ids:
            try:
                ...
                assert graph is not None, f"Failed to create graph for: {loop_id}"
            except Exception as e:
                pytest.fail(f"Failed to create graph for {loop_id}: {e}")
