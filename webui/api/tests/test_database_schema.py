        ...
        with get_test_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                assert new_time > initial_time, (
                    "Trigger did not update modified timestamp"
                )

                # Cleanup
                cur.execute(
                    "DELETE FROM project_ownership WHERE project_id = 'test-project';"
                )
    ...
    def test_jsonb_insert_and_query(self, schema_applied):
        ...
        with get_test_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Cleanup
                cur.execute(
                    "DELETE FROM project_ownership WHERE project_id = 'test-json';"
                )
