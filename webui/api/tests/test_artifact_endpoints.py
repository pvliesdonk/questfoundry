...
class TestCreateArtifact:
    ...
    def test_create_artifact_cold_storage(
        ...
        with patch("webui_api.routers.artifacts.get_storage_backend", return_value=mock_storage):
    ...
    def test_create_artifact_not_owner(
        ...
        with patch("webui_api.routers.artifacts.check_project_ownership", side_effect=raise_403):
            with patch("webui_api.routers.artifacts.get_storage_backend", return_value=mock_storage):
                response = client.post(
                    "/projects/test-project/artifacts",
                    json=sample_artifact_data,
...
class TestStorageBackendSelection:
    ...
    def test_invalid_storage_backend(
        self, client, mock_ownership_check
    ):
        """Test invalid storage backend returns 400."""
        with patch("webui_api.routers.artifacts.get_storage_backend") as mock_get:
            from fastapi import HTTPException
            mock_get.side_effect = HTTPException(status_code=400, detail="Invalid storage backend")
